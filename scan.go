...
// Creates it and enclosing folder if it does exist
...
// dumpStringSliceToFile writes content to the file in path `filePath` (overwritting existing content)
func dumpStringSliceToFile(repos []string, filePath string) {
	content := strings.Join(repos, "\n")
	ioutil.WriteFile(filePath, []byte(content), 0755)
}

// addNewSliceElementsToFile given a slice of strings reprensenting paths, stores them
// to filesystem
